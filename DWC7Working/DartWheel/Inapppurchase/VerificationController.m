...
- (BOOL)doesTransactionInfoMatchReceipt:(NSString*) receiptString
{
    ...
    if ([[UIDevice currentDevice] respondsToSelector:NSSelectorFromString(@"identifierForVendor")]) // iOS 6?
    {
        ...
        // iOS 6 (or later)
       NSString *localIdentifier                   = [[[UIDevice currentDevice] identifierForVendor] UUIDString];
//          NSString *localIdentifier = (NSString *)[[UIDevice currentDevice] identifierForVendor];
        NSString *purchaseInfoUniqueVendorId        = [purchaseInfoFromTransaction objectForKey:@"unique-vendor-identifier"];
        NSString *verifiedReceiptVendorIdentifier   = [verifiedReceiptReceiptDictionary objectForKey:@"unique_vendor_identifier"];
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data
{
    ...
    if (isOk)
    {
        ...
        keyChainMultiArrow = [[KeychainItemWrapper alloc] initWithIdentifier:@"elife" accessGroup:nil];
        
        NSString *strProductId = [[deserializedDictionary objectForKey:@"receipt"] objectForKey:@"product_id"];
        
        if ([strProductId isEqualToString:@"com.globussoft.newlife"]) {
            [self saveLifePurchaseDetails:dict ];
            int lifeCount = 5;
            
            NSString *lifeString = [NSString stringWithFormat:@"%i",lifeCount];
            [[NSUserDefaults standardUserDefaults]setInteger:lifeCount forKey:@"life"];
...
#pragma mark Save To Parse
    -(void) saveLifePurchaseDetails:(NSDictionary *)dict{
        ...
        PFObject *object = [PFObject objectWithClassName: @"LifePurchaseDetails"];
